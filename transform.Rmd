...
Depending on how you view this data frame, it prints a little differently than you might expect. If you're running this code in RStudio's __source pane__, then you'll see a version that allows you to scroll left and right through columns (triangles to the left and/or right of the column names) and up and down through rows ("Previous", numbers, and "Next" at the bottom right):

```{r, echo = FALSE, out.width = NULL}
knitr::include_graphics("screenshots/transform-print.tibble-source.pane.png")
```

RStudio changes the number of columns you can see based on the width of your source pane. In fact, if you make the source pane narrower (by dragging the vertical divider line to the left), RStudio will update the view to show fewer columns:

```{r, echo = FALSE, out.width = NULL}
knitr::include_graphics("screenshots/transform-print.tibble-source.pane-narrow.png")
```


If you're running this code in RStudio's __console pane__, then you'll only see the first few rows and all the columns that fit on one screen:

```{r, echo = FALSE, out.width = NULL}
knitr::include_graphics("screenshots/transform-print.tibble-console.pane.png")
```


RStudio won't automatically resize the columns that you've already printed in the console pane, but it'll print fewer columns if you run `flights` again:

```{r, echo = FALSE, out.width = NULL}
knitr::include_graphics("screenshots/transform-print.tibble-console.pane-narrow.png")
```


Finally, if you're not viewing this code in RStudio at all, but as an HTML file in your browser, it'll look a lot like the console-pane version without color:

```{r, echo = FALSE, out.width = NULL}
knitr::include_graphics("screenshots/transform-print.tibble-knitted.png")
```


These differences in appearance are just that: superficial differences that are only about how `flights` is _printed_. For example, in all 5 screenshots you can see that the entire data frame has 336,776 rows and 19 columns (the first few being `year`, `month`, `day`, `dep_time`, etc.). The underlying data doesn't change, just the way R or RStudio is showing it to you. While this may seem maddening, it's actually an advantage of __tibbles__ like `flights`. Tibbles are data frames, but slightly tweaked to work better in the tidyverse (e.g., when you print data frames that aren't tibbles, you get a huge output that isn't particularly helpful). For now, you don't need to worry about the differences; we'll come back to tibbles in more detail in [wrangle](#wrangle-intro).

<span class="warning">Understanding the difference between what's actually in an object vs. how it's printed is one of the toughest things for people who are new to using R. When you tell R `flights`, you're really telling it "Print `flights` in a way that's suited for the type of thing that `flights` is".</span>
...
Finally, we'll tweak how tibbles are printed to make them a little friendlier to read in the console/HTML views (it won't change the source pane view). We'll override a default so tibbles are printed with un-abbreviated column names. In the HTML file, compare the abbreviated 5th column name above (`sched_dep~1`) with the un-abbreviated column name after we override the default (`sched_dep_time`):

```{r}
##Don't abbreviate column names when printing tibbles
options(pillar.min_title_chars = Inf)
##Print flights again---note column names are un-abbreviated so fewer columns fit
flights
```
...
## `filter()` and `slice()`: Return a subset of rows
...
Finally, `slice()` allows you to subset observations based on their row number:

```{r}
slice(flights, 10000:10005)
```

This is occasionally useful when you're just exploring your data, but you should almost never count on it when you're formulating data-wrangling code. Row numbers can change, so any code that relies on them to uniquely identify observations is easily breakable. It also makes for code that's harder to understand; when you look at your code 6 months later, it's easy to work backwards and figure out why you `filter()`ed by a particular attribute, but very difficult to identify why you `slice()`d by some random row numbers!

If you simply want a random sampling of rows, use `slice_sample()`. If you want to ensure that you get the _same_ random sampling each time, set a random seed beforehand:
...
## `select()`, `rename()`, and `relocate()`: Operate on existing variables {#select}
...
`select()` can be used to rename variables, but this functionality is rarely useful because it drops all of the variables not explicitly mentioned. Instead, use `rename()`, which is a variant of `select()` that keeps all the variables that aren't explicitly mentioned:

```{r}
rename(flights, tail_num = tailnum)
```

Another option is to use `relocate()`, which moves variables elsewhere in the data frame:

```{r}
##Move time_hour & air_time after month
relocate(flights, time_hour, air_time, .after=month)
##Move time_hour & air_time to the left of the data frame (before the current 1st column)
relocate(flights, time_hour, air_time, .before=1)
```
...
## `mutate()` and `transmute()`: Add new variables or change existing variables

Besides selecting sets of existing columns, it's often useful to add new columns that are functions of existing columns. That's the job of `mutate()`. 

By default, `mutate()` adds new columns at the end of your dataset so we'll start by creating a narrower dataset so we can see the new variables. Remember that when you're in RStudio, the easiest way to see all the columns is `View()`.
...
In addition to adding _new_ variables, `mutate()` and `transmute()` can also change _existing_ variables. For example, you can convert `distance` from miles to kilometers:

```{r}
mutate(flights_sml,
       ##1.609344 km in a mile
       distance = 1.609344 * distance)
```

Note that when `mutate()` changes existing variables, it keeps the changed variable in place (as opposed to new variables, which go at the end of the dataset)
...
## `summarise()`, `group_by()`, and `ungroup()`: Collapse the data frame to a row per group
...
<span class="warning">
You might've noticed the message `summarise() has grouped output by 'year', 'month'. You can override using the .groups argument`. Since `summarise()` is typically used with groups, it has special ways to handle data with multiple groups; for more details, see `?summarise` under the "Arguments" heading. If the message annoys you, add the argument `.groups = "drop_last"` to `summarise()`
</span>


Together `group_by()` and `summarise()` provide one of the tools that you'll use most commonly when working with dplyr: grouped summaries. But before we go any further with this, we need to introduce a powerful new idea: the [pipe](#pipes).
...
<span class="warning">
The `%>%` pipe is not part of base R. It was introduced by the `magrittr` package---the name is a reference to René Magritte's painting [The Treachery of Images](https://en.wikipedia.org/wiki/The_Treachery_of_Images). The pipe has since been adopted by all the `tidyverse` packages, so when you load `library(tidyverse)`, the `%>%` pipe is automatically loaded too.<br>As of R version 4.1, base R also now has a 'native' pipe operator `|>`, which works slightly differently than the `%>%` pipe. We'll be working with the `%>%` pipe because the base R pipe is fairly new and the `%>%` pipe is so established; however, you should be aware of the `|>` pipe when you're googling pipes in R.
</span>
...
<span class="warning">
By default, `summarise()` uses `.groups = "drop_last"`, which peels off one level of the grouping. This isn't true if you use a different setting for `.groups`, though.
</span>
