...
For list elements that are atomic vectors, `str()` prints a compact view:

```{r}
set.seed(12345)
x6 <- list(a = 1:10000,
           b = sample(LETTERS, 200, replace=TRUE))
str(x6)
```

By default, `str()` prints all sub-lists (list elements that are lists) at all depths, but you can use the `list.len` and `max.level` arguments to make this more compact:

```{r}
str(gh_users, max.level = 1)
str(gh_users, max.level = 2, list.len = 4)
```

`str()` also prints attributes (metadata) by default, but you can suppress this with `give.attr = FALSE`:

```{r}
x7 <- read_csv("https://github.com/djvill/r4ds/raw/main/data/01-sales.csv",
               show_col_types = FALSE)
str(x7, give.attr = FALSE)
```

As lists get even larger and more complex, `str()` eventually starts to fail, and you may want to switch to `View()`[^rectangling-1].
...
[^rectangling-1]: `View()` only works in RStudio.
...
Finally, you can use `pluck()` to access list elements by index or (if applicable) name.
You can supply a series of accessors to go deeper into the list's hierarchy.

```{r}
pluck(x5, 2, 2, 2)
```

`pluck()` provides a more consistent way to specify accessors than in base R (see @sec-subset-one), which uses a mix of `[[` and `$`

```{r}
identical(pluck(gh_repos, 1, 1, "owner", 1),
          gh_repos[[1]][[1]]$owner[[1]])
```

The combination of `pluck()` and `str()` can be useful way to explore lists, especially if you're not using RStudio (so `View()` isn't an option):
...
```{r}
# A path to a json file inside the package (it'll look different on different computers):
gh_users_json()
