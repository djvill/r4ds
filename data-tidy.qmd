...
We haven't yet discussed how to handle the situation of multiple values per cell, such as `table3`:

```{r}
table3
```

For situations like these, we use functions from the `tidyr::separate_*()` family, such as `separate_wider_delim()`.
These functions will make more sense once we've introduced strings in @sec-strings and regular expressions in @sec-regular-expressions, so we'll return to them then.
...
Moreover, what counts as an observation can differ from one situation to the next;
this is part of what motivates the idea of relational data and joins, which you'll learn more about in @sec-joins.
