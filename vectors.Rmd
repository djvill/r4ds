...
<span class="warning">
(Dan) We'll talk about strings in a few weeks!
</span>
...
<span class="warning">
Tidyverse functions tend to be stricter about inputs so you don't accidentally use the wrong inputs (we've already seen this with `pivot_longer()` and data types). Compare the following:
</span>
    
```{r, error=TRUE}
##base ifelse: Okay with mixing character & logical
ifelse(2 < 3, "Less", NA)

##dplyr ifelse: Not okay with mixing character & logical
dplyr::if_else(2 < 3, "Less", NA)

##To avoid error, specify which NA:
dplyr::if_else(2 < 3, "Less", NA_character_)
```
...
<span class="warning">
Not all the base R `is.*()` functions have `purrr::is_*()` counterparts (e.g., `is.na()`).
</span>
...
1.  A numeric vector containing only integers. The integers must either be all 
    ...
    <span class="warning">
    FYI, `character(0)` or `character(0L)` just means "a character vector of length zero". Atomic vectors can have types even if there's nothing in them!
    </span>
...
1.  Draw the following lists as nested sets (by hand):
...
<span class="warning">
(Dan) Feel free to skip or skim this section
</span>
...
<span class="warning">
(Dan) Feel free to skip or skim this section, except for the subsection on tibbles
</span>
