...
::: callout-warning
If you're used to R Markdown, please try Quarto for at least 2 weeks' worth of assignments.
If you can't get used to the differences, you can switch back to R Markdown.
:::
...
::: callout-warning
Download the file (<https://github.com/djvill/r4ds/blob/main/quarto/diamond-sizes.qmd>) and open it in RStudio.
Make sure the editor is on "Source" mode rather than "Visual".
:::
...
::: callout-warning
I much prefer the first option, "Chunk Output Inline", since it's easier to keep track of which code produced which output.
...
This will create an HTML file, `diamond-sizes.html`, that you can open in your browser, as shown in @fig-diamond-sizes-report-viewer.

```{r}
#| label: fig-diamond-sizes-report-viewer
#| echo: false
#| out-width: "90%"
#| fig-cap: |
#|   A rendered Quarto document in the Chrome browser.
knitr::include_graphics("quarto/diamond-sizes-report-viewer.png")
```

If you would prefer to display the report in the Viewer pane, you can click on the gear icon next to "Render" and switch to "Preview in Viewer Pane".
Then when you render (click "Render" or press Cmd/Ctrl + Shift + K), the report will appear as shown in @fig-diamond-sizes-report.
...
::: callout-warning
Please don't use Visual editor.
It reformats your code in annoying ways, and it's not an effective bridge to learning how to author literate-programming documents.
I'm leaving this section in for your reference.
:::
...
1.  Modify `diamond-sizes.qmd` to add a section that explores how diamond sizes vary by cut, color, and clarity.
...
2.  Modify `diamond-sizes.qmd` to add a section that describes the largest 20 diamonds, including a table that displays their most important attributes.
...
To embed an image from an external file, use the syntax `![OPTIONAL CAPTION](PATH TO FILE)`.
The path can be a file on your computer (path relative to the directory where the Quarto document is saved), or a URL.
...
1.  Open `diamond-sizes.qmd`, find an image of a diamond, copy it, and paste it into the document. Double click on the image and add a caption. Resize the image and render your document. Observe how the image is saved in your current working directory.
...
They can be markdown tables that you create directly in your Quarto document (using [Markdown table syntax](#sec-source-editor)) or they can be tables generated as a result of a code chunk.
...
1.  Open `diamond-sizes.qmd`, insert a code chunk, and add a table with `knitr::kable()` that shows the first 5 rows of the `diamonds` data frame.
...
::: callout-warning
I would recommend using this setting while you're still learning Quarto, since it makes it easier to send output HTML documents.
:::
