...
-   **Tabular data** is a set of values, each associated with a variable and an observation.
    Tabular data is *tidy* if each value is placed in its own "cell", each variable in its own column, and each observation in its own row---more on that later (@sec-data-tidy).
...
For the remaining plots in this chapter, the book will suppress this warning so it's not printed alongside every single plot you read.
