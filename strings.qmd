...
We'll then discuss tools that work with individual characters.
The chapter finishes with a brief discussion of where your expectations from English might steer you wrong when working with other languages.
...
If you forget to close a string, you'll see `+`, the continuation prompt, in the R console:
...
In RStudio, if your Quarto code chunk has an unclosed string, you might see no error, an error like `Error: Incomplete expression: "This is a string without a closing quote`, or an error like this:
...
Beware that the printed representation of a string (i.e., what you see when you just run the name of the string) is not the same as the string itself.
When R prints a string, it shows the escapes so that you can copy and paste the output to recreate that string.
...
::: callout-warning
Don't worry too much about the distinction between "the printed representation of a string" and "the string itself" if it's confusing!
:::

### Raw strings {#sec-raw-strings}

::: callout-warning
I've never used raw strings (they're fairly new in R), but I'm leaving this in for your reference.
:::
...
Note that `str_view()` uses curly braces for tab characters to make them easier to spot[^strings-3].
...
```{r}
str_flatten(c("lions", "tigers", "bears", "oh my!"))
str_flatten(c("lions", "tigers", "bears", "oh my!"), " and ")
str_flatten(c("lions", "tigers", "bears", "oh my!"), " and ", last = ", ")
```
...
In this section, we'll introduce you to functions that allow you to work with the individual characters within a string.
You'll learn how to find the length of a string, extract substrings, and handle long strings in plots and tables.

### Length

`str_length()` tells you the number of characters in the string[^strings-7]:

[^strings-7]: Most of the time; see [below](#letter-variations)

```{r}
str_length(c("a", "R for data science", "", NA))
```

You could use this with `count()` to find the distribution of lengths of US babynames and then with `filter()` to look at the longest names, which happen to have 15 characters[^strings-8]:

[^strings-8]: Looking at these entries, we'd guess that the babynames data drops spaces or hyphens and truncates after 15 characters.
...
We could use `str_sub()` with `mutate()` to find the first and last character of each name:
...
2.  Use `str_length()` and `str_sub()` to extract the middle character from each baby name. What will you do if the string has an even number of characters?
3.  Are there any major trends in the length of babynames over time? What about the popularity of first and last characters?
...
Each of these six hexadecimal numbers represents one character: `48` is H, `61` is a, and so on.
...
For example here are two inline CSVs with unusual encodings[^strings-9]:

[^strings-9]: Here I'm using the special `\x` to encode binary data directly into a string.
...
### Letter variations {#letter-variations}

Working in languages with accents poses a significant challenge when determining the position of characters (e.g., with `str_length()` and `str_sub()`) as accented letters might be encoded as a single individual character (e.g., ü) or as two characters by combining an unaccented character (e.g., u) with a diacritic mark (e.g., ¨).
...
Sorting strings depends on the order of the alphabet, and the order of the alphabet is not the same in every language[^strings-10]!
