...
In this class, we'll mostly use Quarto documents (@sec-quarto) rather than scripts.
These have a few benefits compared to scripts: they display code and output interleaved (@fig-diamond-sizes-notebook), they can be rendered to output formats (@fig-diamond-sizes-report), and they have a predictable working directory.
However, the same general principles apply to both.
:::

### RStudio diagnostics

In the script editor, RStudio will highlight syntax errors with a red squiggly line and an X in the sidebar:
...
In RStudio, open the Options window (*Tools* menu \> *Global Options...*) and mimic the options shown in @fig-blank-slate.
...
#| echo: false
#| fig-cap: |
#|   Copy these options in your RStudio options (*Tools* menu \> *Global Options...*) to always start your 
...
If you're planning on using Git to version-control your project, remember: Never put Git directories in a location that will be automatically backed up to the cloud.
:::
...
::: callout-warning
This is part of the reason we'll use Quarto documents (@sec-quarto) in this class rather than scripts.
The working directory of a Quarto document is wherever that document lives, which only changes if you move the file.
The working directory of a script is wherever the script was *called from*, which can change easily (especially if you're not always working out of RStudio).
:::
...
But as you accumulate more code across multiple projects, you'll learn to appreciate how a little upfront organisation can save you a bunch of time down the road.
...
So in the next chapter, you're going to learn how to load data from disk into your R session using the readr package.
