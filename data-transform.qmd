...
In this section, we'll focus on the most important functions: `summarize()` and the slice family of functions.
...
For example, we could calculate the average delay for each date
...
`.by` uses tidyselect semantics, so you can use any of the operators that `select()` uses (see [above](#sec-select)).
...
```{r}
flights |> 
  slice_max(arr_delay, n = 1, by = dest) |>
  relocate(dest)
```
...
::: callout-warning
When _R for data science_ was first published, `.by` had just been introduced to dplyr in version 1.1.0.
Although `.by` is still marked as an ["experimental" feature](https://lifecycle.r-lib.org/articles/stages.html#experimental), I have adopted it wholesale in my own coding practice, as I find it far more useful than `group_by()`.
I'm leaving in the `group_by()` section here for reference.
You can read more about `.by` vs. `group_by()` here: <https://dplyr.tidyverse.org/reference/dplyr_by.html>.
:::
...
`group_by()` adds this grouped feature (referred to as a class) to the data frame, which changes the behavior of the subsequent verbs applied to the data.
Thus, the main advantage of `group_by()` over `.by` is when you want to apply multiple verbs in a row to the same grouping structure.

You can then use `ungroup()` to remove grouping from a dataframe:
...
1.  Which carrier has the worst average delays?
    ...
    Why/why not?
    (Hint: think about `flights |> summarize(n(), .by=c(carrier, dest))`)
...
6.  Suppose we have the following tiny data frame:
    ...
    a.  Write down what you think the output will look like, then check if you were correct, and describe what `arrange()` does.
    ...
    b.  Write down what you think the output will look like, then check if you were correct, and describe what `summarize()` with `group_by()` does.
    ...
    c.  Write down what you think the output will look like, then check if you were correct, and describe what `summarize()` with `.by` does.
        How is the output different from the one in part (b)?
    ...
    d.  Write down what you think the output will look like, then check if you were correct, and describe what the pipeline does.
        How is the output different from the one in parts (b) and (c)?
...
The tools are roughly grouped into three categories: those that manipulate the rows (like `filter()` and `arrange()`), those that manipulate the columns (like `select()` and `mutate()`) and those that manipulate groups (like `summarize()`).
