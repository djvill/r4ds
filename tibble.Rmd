...
<span class="warning">`iris` is one of the datasets built-into R (without needing to load any data or packages). You can check out all the datasets available from all loaded packages by running `data()`</span>
...
<span class="warning">
`data.frame()` does _not_ allow you to refer to variables that you just created. Try running the code above with `data.frame()` in place of `tibble()` to see for yourself.
</span>
...
* Use `options(pillar.min_title_chars = Inf)` to never abbreviate column
  names, regardless of their length (`pillar` is imported automatically when 
  you load `tibble`)
...
<span class="warning">
A better option is to use `dplyr`'s `pull()`, which works like `$` in that it extracts by name: `df %>% pull(x)` is the same as `df$x`. The `magrittr` package also has the `[[` alias `extract2()` (2 for a double square bracket): `df %>% magrittr::extract2("x")` is the same as `df[["x"]]`.
</span>
